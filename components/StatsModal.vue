  ...
  <div class="fixed inset-0 flex items-center justify-center p-4 modal-overlay" data-modal="true">
    <div class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col modal-content">
      ...
      <div class="flex items-center justify-between p-6 border-b border-gray-100 modal-header flex-shrink-0">
        <h2 class="text-2xl font-bold text-gray-900">{{ $t('modals.stats.title') }}</h2>
        <button 
          @click="$emit('close')"
      ...
      <div class="modal-body scrollable flex-1 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div class="card text-center">
            ...
            <p class="text-gray-600">{{ $t('modals.stats.totalPosts') }}</p>
          </div>
          ...
          <div class="card text-center">
            ...
            <p class="text-gray-600">{{ $t('modals.stats.recentActivity') }}</p>
          </div>
          ...
          <div class="card text-center">
            ...
            <p class="text-gray-600">{{ $t('modals.stats.activeUsers') }}</p>
          </div>
          ...
          <div class="card text-center">
            ...
            <p class="text-gray-600">{{ $t('modals.stats.totalComments') }}</p>
          </div>
        </div>

        <!-- 情绪分布 -->
        <div>
          <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('modals.stats.emotionDistribution') }}</h3>
        ...
        <!-- 热门标签 -->
        <div>
          <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('modals.stats.hotTags') }}</h3>
          ...
          <div v-else class="text-center py-6 text-gray-500">
            {{ $t('modals.stats.noTagData') }}
          </div>
        </div>

        <!-- 时间趋势 -->
        <div>
          <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('modals.stats.publishTrend') }}</h3>
...
const emotionConfig = computed(() => ({
  happy: { label: $t('emotions.happy'), icon: 'mdi:emoticon-happy', color: 'bg-green-500' },
  sad: { label: $t('emotions.sad'), icon: 'mdi:emoticon-sad', color: 'bg-blue-500' },
  angry: { label: $t('emotions.angry'), icon: 'mdi:emoticon-angry', color: 'bg-red-500' },
  anxious: { label: $t('emotions.anxious'), icon: 'mdi:emoticon-confused', color: 'bg-yellow-500' },
  excited: { label: $t('emotions.excited'), icon: 'mdi:emoticon-excited', color: 'bg-pink-500' },
  frustrated: { label: $t('emotions.frustrated'), icon: 'mdi:emoticon-cry', color: 'bg-gray-500' },
  grateful: { label: $t('emotions.grateful'), icon: 'mdi:emoticon-kiss', color: 'bg-purple-500' },
  neutral: { label: $t('emotions.neutral'), icon: 'mdi:emoticon-neutral', color: 'bg-indigo-500' }
}))
...
const getEmotionColor = (emotion: EmotionType): string => {
  return emotionConfig.value[emotion]?.color || 'bg-gray-500'
}
